...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setDefaultDate();
  renderAll();
  // Dismiss loader after first render
...
/* ════ ANIMATE NUMBER WITH COUNTER ════ */
function animateCounter(el, target, dur = 580) {
  if (!el) return;
  ...
  (function step(now) {
    ...
    el.textContent = fmtCur(from + (target - from) * ease);
...
function attachRippleEffects() {
  document.querySelectorAll('.btn-primary, .topbar-add-btn, .gc-add-funds-btn, .manage-wallets-btn').forEach(btn => {
